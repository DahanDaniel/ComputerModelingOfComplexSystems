...
class Birds:
    def __init__(self, withPredator=False):
        self.positions = np.random.uniform(0, L, N) + 1j * np.random.uniform(0, L, N)
        ...
        self.neighboursTree = cKDTree(positions2D, boxsize=[L, L])
        self.neighboursDistances = np.array(
            self.neighboursTree.sparse_distance_matrix(
                self.neighboursTree, max_distance=R, output_type="coo_matrix"
            ).todense()
        ).ravel()
        ...
        if self.withPredator:
            self.predatorPos = np.random.uniform(0, L) + 1j * np.random.uniform(0, L)
            closestVictimIdx = self.neighboursTree.query(
                [self.predatorPos.real, self.predatorPos.imag], 1
            )[1]
            ...
            #                             - self.predatorPos)
            self.predatorDir = self.directions[
                closestVictimIdx
    ...
    def new_directions(self):
        ...
        new_directions = np.angle(sum_velocities) + A * np.random.uniform(
            -np.pi, np.pi, N
        )
    ...
    def update(self):
        ...
        if self.withPredator:
            ...
            threatenedBirds = np.where(
                ...
                np.abs(self.positions - self.predatorPos)
                < RB
            )
            threatenedDir = np.angle(self.positions[threatenedBirds] - self.predatorPos)
    ...
    def save_frame(self):
        ...
        while os.path.exists(
            os.path.join(PATH_PREFIX, r"starlingsSimulation\\frame%s.png" % nameIndex)
        ):
            i += 1
        ...
        figName = os.path.join(
            PATH_PREFIX, r"starlingsSimulation\\frame%s.png" % nameIndex
        )
